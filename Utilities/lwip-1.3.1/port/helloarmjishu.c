...
static err_t helloarmjishu_recv(void *arg, struct tcp_pcb *pcb, struct pbuf *p, err_t err);
static err_t helloarmjishu_accept(void *arg, struct tcp_pcb *pcb, err_t err);
static void helloarmjishu_conn_err(void *arg, err_t err);
...
{     
  ...
  printf("\n\rTelnet HelloArmjishu_accept:%d.%d.%d.%d  %s", (u8_t)(IPaddress),(u8_t)(IPaddress >> 8),(u8_t)(IPaddress >> 16),(u8_t)(IPaddress >> 24),GREETING);
  ...
  tcp_err(pcb, helloarmjishu_conn_err);
  tcp_recv(pcb, helloarmjishu_recv);
...
void helloarmjishu_init(void)
{
  ...
  tcp_accept(pcb, helloarmjishu_accept);   
